...
ordered quantity. We therefore applied one-hot encoding with \texttt{drop=first}, representing the 20 stations by 19 binary columns and one reference category \cite{sklearn_ohe}.
...
\midrule
Station identity & 
19 one-hot binary columns from \texttt{start\_station\_id}. The 20th station is kept as the reference category, so station differences are represented without implying a numeric order or adding a redundant dummy column. \\
\addlinespace
Weather \& daylight & 
