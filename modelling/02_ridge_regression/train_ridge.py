...
# We test a small grid of alpha values.
# Alpha controls how strongly the coefficients are regularized.
...
def save_model_info(feature_names: list[str], metrics: dict, best_alpha: float) -> None:
    """
    Save a summary of the final ridge model setup.
...
def main() -> None:
    ...
    if use_encoded:
        ...
        scaler, feature_names, X_train_ready, X_val_ready, X_test_ready = (
            prepare_encoded_feature_matrices_for_model(
                ...
                target_col=TARGET_COL,
                feature_names=None,
                scale_numeric=True,  # Ridge regression needs scaling
            )
        )
        ...
        # - numeric features are scaled because ridge regression uses distances in parameter space
        (
            preprocessor,
            feature_names,
            X_train_ready,
    ...
    # We train one ridge model per alpha value and compare validation performance.
