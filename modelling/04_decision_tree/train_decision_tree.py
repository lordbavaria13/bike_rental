...
# The goal is to control tree complexity and reduce overfitting.
PARAM_GRID = [
...
def save_model_info(feature_names: list[str], metrics: dict, best_params: dict) -> None:
    """
    Save a summary of the final decision tree setup.

    This file documents:
    - which transformed features were used
    - which tree parameters were selected
    ...
    model_info = {
        ...
        "model_file": str(MODEL_DIR / "decision_tree.joblib"),
...
def plot_depth_search(search_df: pd.DataFrame) -> None:
    """
    Plot validation RMSE for all tested tree settings.

    We create string labels first so matplotlib does not mix numeric
    and categorical axis types.
    """
    ...
    plt.title(f"{MODEL_NAME} - Validation RMSE by Tree Setting", fontsize=TITLE_SIZE)
    plt.xlabel("parameter setting", fontsize=LABEL_SIZE)
    plt.ylabel("Validation RMSE", fontsize=LABEL_SIZE)
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "validation_curve_depth.png", dpi=DPI)
    plt.close()
...
def main() -> None:
    ...
    # - numeric features are not scaled for a decision tree
    (
        preprocessor,
        feature_names,
        X_train_ready,
    ...
    # We train one tree for each parameter setting and compare validation performance.
